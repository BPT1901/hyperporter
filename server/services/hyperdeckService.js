...
class HyperdeckService extends EventEmitter {
  ...
  parseResponse(response) {
    ...
    if (response.startsWith("100 syntax error")) {
      console.log(
        "Warning: Received syntax error from HyperDeck, continuing to process response",
      );
    }
    ...
    if (response.includes("transport info:")) {
      ...
      const timecodeMatch = response.match(
        /timecode: (\d{2}:\d{2}:\d{2}:\d{2})/,
      );
    ...
    if (response.includes("slot id:")) {
      ...
      const statusMatch = response.match(/status: (\w+)/);
      const recordingMatch = response.match(
        /recording time: (\d+:\d+:\d+:\d+)/,
      );
  ...
  processClipResponse(line) {
    ...
    if (line.startsWith("200") || line.startsWith("500")) {
      if (this.clipList.length > 0) {
        console.log(
          "Command complete, emitting list of",
          this.clipList.length,
          "clips",
        );
  ...
  async getClipList(slot) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const handleResponse = (response) => {
        ...
        const clipMatch = response.match(
          /^(\d+): (.+) H\.264 .+ (\d{2}:\d{2}:\d{2}:\d{2})/,
        );
        if (clipMatch) {
      ...
      this.sendCommand(`disk list: slot id: ${slot}`).catch((error) => {
